...
public:
    ...
    {
        mcdm_status_t ret = OK;
        mcdm_SessionId_t sessionId = mcdm_SessionId_t((const char*)aMetadata->KeyId(),
                                                      aMetadata->KeyIdSize());
        ...
        uint8_t key[16] = {0};
        size_t numSubSamples = aMetadata->NumSubsamples();
        mcdm_subsample_t *subSamples = new mcdm_subsample_t[numSubSamples];
...
private:
    ...
    class MessageTask : public GMPTask {
        ...
        virtual void Run() override {
            mCallback->SessionMessage(mSessionId.c_str(), (uint32_t)mSessionId.size(),
                    (const uint8_t*)mMessage.c_str(), (uint32_t)mMessage.size(),
                    "", 0);
        }
        GMPDecryptorCallback* mCallback;
        string mSessionId;
