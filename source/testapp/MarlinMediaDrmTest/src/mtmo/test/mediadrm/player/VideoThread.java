...
public class VideoThread extends Thread {
    private static final Logger mLogger = Logger.createLog(VideoThread.class
            .getSimpleName());
    ...
    private Player.MediaInfo mMediaInfo;
    ...
    public VideoThread(Surface surface, Player.MediaInfo mediaInfo) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int j = 0; j < supportedTypes.length; ++j) {
                if (supportedTypes[j].equalsIgnoreCase(mime)) {
                    mLogger.d("getSecureDecoderNameForMime returned: "
                                    + info.getName() + ".secure");
    ...
    public void run() {
        ...
        try {
            ...
            if (mMediaInfo.isProtected) {
                mLogger.d("psshsize: " + mMediaInfo.video_sinf.length);
                info = new CryptoInfo();
                mediaCrypto = new MediaCrypto(mMediaInfo.uuid,
                        mMediaInfo.getVideoSinf());
            }

            for (int i = 0; i < extractor.getTrackCount(); i++) {
                MediaFormat format = extractor.getTrackFormat(i);
                String mime = format.getString(MediaFormat.KEY_MIME);
                format.setInteger("nalSize", 4);
                if (mime.startsWith("video/")) {
                    int height = format.getInteger(MediaFormat.KEY_HEIGHT);
                    extractor.selectTrack(i);
                    mLogger.d("Mime: " + mime);
                    mLogger.d("Height: " + height);
                    if (mMediaInfo.isProtected) {
