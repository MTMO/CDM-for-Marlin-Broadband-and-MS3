...
public class AudioThread extends Thread {
    private static final Logger mLogger = Logger.createLog(AudioThread.class
            .getSimpleName());
    ...
    public AudioThread(Player.MediaInfo mediaInfo) {
    ...
    public void run() {
        ...
        try {
            ...
            if (mMediaInfo.isProtected) {
                info = new CryptoInfo();
                mLogger.d("psshsize: " + mMediaInfo.audio_sinf.length);

                mLogger.d("### new MediaCrypto");
                mediaCrypto = new MediaCrypto(mMediaInfo.uuid,
                        mMediaInfo.getAudioSinf());
            }
            ...
            while (!Thread.interrupted()) {
                if (!isEOS) {
                    ...
                    int inIndex = decoder.dequeueInputBuffer(Player.TIMEOUT_US);
                    if (inIndex >= 0) {
